                            ...
                            switch (request->getMessageType()) {
                                case RequestModel_t::UpdateGameStateRequestModel: {
                                    ...
                                    // sending damaged players info
                                    {
                                        ...
                                        std::optional<response_models::DamagedPlayersResponseModel> optionalModel = interactor.execute(
                                                *gameSession);

                                        if (optionalModel.has_value()) {
                                            response_models::DamagedPlayersResponseModel responseModel = *std::move(
                                                    optionalModel);

                                            // serialize
                                            std::unique_ptr<char[]> buffer_ptr(new char[responseModel.ByteSizeLong()]);
                                            responseModel.SerializeToArray(buffer_ptr.get(),
