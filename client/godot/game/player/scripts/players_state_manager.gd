...
#var players_specializations = {} # stores ints (types of players specializations, for lazy updating)
var players_data: Dictionary = {
	# id: { username, specialization, current_hitpoint, initial_hitpoints, team_id, kills, deaths }
}

func set_default_data(player_id):
	players_data[player_id] = {
		# username, specialization, current_hitpoint, initial_hitpoints, team_id
		"username": "",
		"specialization": -1,
		"initial_hitpoints": 0,
		"current_hitpoints": 0,
...
func set_main_player_data(main_player):
	players_data[main_player.player_id] = {
		"username": "",
		"specialization": -1,
		"initial_hitpoints": 0,
		"current_hitpoints": 0,
...
#
#				if (!players_data.has(main_player.player_id)):
#					set_default_data(main_player.player_id)
#
#				players_data[main_player.player_id].specialization = players_specializations.get(main_player.player_id)
#				players_specializations.erase(main_player.player_id)
		else:
			var player_id = model.get_player_id()
#			if (!players_data.has(player_id)):
#					set_default_data(player_id)
...
#					'initial_hitpoints': players_data[player_id].initial_hitpoints,
#					'current_hitpoints': players_data[player_id].current_hitpoints
#				}
...
#			var player = players_parent_node.get_node(str(player_id))
#			if (players_specializations.has(player_id)):
#				player.update_player_specialization(
#					players_specializations.get(player_id)
#				)
#				players_data[player_id].specialization = players_specializations.get(player_id)
#				players_specializations.erase(player_id)
...
func update_damaged_players_states(damaged_players: Array, main_player, players_parent_node):
	for i in range(damaged_players.size()):
		...
		if (!players_data.has(defender_id)):
			set_default_data(defender_id)
...
func update_killed_players_states(killed_players: Array, main_player, players_parent_node):
	for i in range(killed_players.size()):
		...
		if (!players_data.has(killed_id)):
			set_default_data(killed_id)
...
#		var player_id = hitpoints[i].get_player_id()
#		if (!players_data.has(player_id)):
#			set_default_data(player_id)
#		players_data[player_id].initial_hitpoints = hitpoints[i].get_initial_hitpoints()
#		players_data[player_id].current_hitpoints = hitpoints[i].get_current_hitpoints()
