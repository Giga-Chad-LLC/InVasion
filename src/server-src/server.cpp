...
namespace invasion::session {
    ...
    void initializeUserForFrontend(game_models::GameSession &session, std::shared_ptr<User> user) {
        // send to user his ID
        std::cout << "Send user his ID: " << user->getClientId() << std::endl;
    ...
    void Server::waitNewUser() {
        while (true) {
            ...
            if (!ImplementedDispatherEachSender && baseUsers.size() ==
                                                   TOTAL_CLIENTS_COUNT_IN_SESSION) { // создание обработчика, если комманда собралась пока что handler - заглушка
